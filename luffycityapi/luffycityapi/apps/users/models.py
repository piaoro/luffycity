...
from luffycityapi.utlis.models import BaseModel
...
class Credit(BaseModel):
    ...
    number = models.IntegerField(default=0,verbose_name="积分数量",help_text="""
        如果是扣除积分则需要设置积分为负数，如果消费10积分，则填写-10，<br>如果是添加积分则需要设置积分为正数，如果获得10积分，则填写10。
    """)
    ...
    def __str__(self):
        if self.number > 0:
            ...
            oper_text = "减少"
        return f"{self.get_operation_display()} {self.created_time.strftime('%Y-%m-%d %H:%M:%S')} 用户{self.user.username} {oper_text} {abs(self.number)}积分"
