...
class Coupon(BaseModel):
    ...
    coupon_type = models.SmallIntegerField(choices=type_choices, default=0, verbose_name="优惠劵类型")
    ...
    condition = models.IntegerField(blank=True, default=0, verbose_name="满足使用优惠劵的价格条件")
    per_limit = models.SmallIntegerField(default=1, verbose_name="每人领取限制")
...
class CouponDirection(models.Model):
    ...
    coupon = models.ForeignKey(Coupon, on_delete=models.CASCADE, related_name="to_direction", verbose_name="优惠劵",
                               db_constraint=False)
    created_time = models.DateTimeField(auto_now_add=True, verbose_name="添加时间")

    class Meta:
        db_table = "fg_coupon_course_direction"
        verbose_name = "优惠劵与学习方向"
        verbose_name_plural = verbose_name
...
class CouponCourseCat(models.Model):
    ...
    coupon = models.ForeignKey(Coupon, on_delete=models.CASCADE, verbose_name='优惠劵', related_name="to_category",
                               db_constraint=False)
    created_time = models.DateTimeField(auto_now_add=True, verbose_name="添加时间")

    class Meta:
        db_table = 'fg_coupon_course_category'
        verbose_name = "优惠劵与课程分类"
        verbose_name_plural = verbose_name
...
class CouponCourse(models.Model):
    course = models.ForeignKey(Course, on_delete=models.CASCADE, related_name='to_coupon', verbose_name="课程",
                               db_constraint=False)
    coupon = models.ForeignKey(Coupon, on_delete=models.CASCADE, related_name="to_course", verbose_name="优惠劵",
                               db_constraint=False)
    created_time = models.DateTimeField(auto_now_add=True, verbose_name="添加时间")

    class Meta:
        db_table = 'fg_coupon_course'
        verbose_name = "优惠劵与课程信息"
        verbose_name_plural = verbose_name
...
class CouponLog(BaseModel):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="to_coupon", verbose_name='用户',
                             db_constraint=False)
    coupon = models.ForeignKey(Coupon, on_delete=models.CASCADE, related_name="to_user", verbose_name='优惠劵',
                               db_constraint=False)
