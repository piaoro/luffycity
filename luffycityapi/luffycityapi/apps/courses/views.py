...
from .serializers import CourseDirectionModelSerializer, CourseDirection, CourseCategoryModelSerializer, CourseCategory, \
    Course, CourseInfoModelSerializer, CourseIndexHaystackSerializer,CourseRetrieveModelSerializer,CourseChapter,CourseChapterModelSerializer
...
class HotWordAPIView(APIView):
    ...
    def get(self, request):
        ...
        # 根据date_list找到最近指定天数的所有集合，并完成并集计算，产生新的有序统计集合
        redis.zunionstore(constants.DEFAULT_HOT_WORD, date_list, aggregate="sum")  # 按分数store进行倒序显示排名靠前的指定数量的热词
...
class CourseChapterListAPIView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        try:
            ...
            return []
        queryset = CourseChapter.objects.filter(course=course,is_show=True, is_delete=False).order_by("orders", "id")
        return queryset.all()
